...
function formatMetadataValue(value: unknown): string {
  ...
  if (typeof value === 'object') {
    ...
    return entries.map(([key, entryValue]) => `${auditLabel(key)}: ${formatMetadataValue(entryValue)}`).join('; ');
  }
  return String(value);
}

export function auditMetadataLines(value?: Record<string, unknown> | null) {
...
export function metadataPreview(log: AuditLog) {
  ...
  if (bits.length) return bits.map((bit) => String(bit).replace(/_/g, ' ')).join(' · ');
  return auditMetadataLines(value)
    .slice(0, 3)
