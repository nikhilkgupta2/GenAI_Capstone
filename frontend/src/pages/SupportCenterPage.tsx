...
import { getSupportIssues } from '../lib/super-admin-api';
...
export function SupportCenterPage() {
  ...
  const handleResolveIssue = (id: string) => {
    ...
    setNotice('Support ticket marked as resolved.');
  };

  const openIssues = data.reported_issues.filter(
            ...
            openIssues.map((issue) => (
              <section key={issue.id} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-wrap justify-between items-start gap-3 border-b border-slate-100 pb-3 mb-3">
                  <div>
                    <h3 className="font-bold text-slate-900 text-base">{issue.category}</h3>
                    <p className="text-xs text-slate-500">
                      Reported by <span className="font-semibold">{issue.company_name}</span> ({issue.contact_email})
                    </p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      className="inline-flex items-center rounded bg-emerald-600 px-2.5 py-1 text-xs font-semibold text-white hover:bg-emerald-500 transition"
