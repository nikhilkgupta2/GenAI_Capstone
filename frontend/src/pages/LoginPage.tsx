...
export function LoginPage() {
  ...
  const verificationSuccess = Boolean((location.state as { verificationSuccess?: boolean } | null)?.verificationSuccess);
  ...
  return (
    ...
    >
      <form className="space-y-4" onSubmit={form.handleSubmit((values) => mutation.mutate(values))}>
        {registrationSuccess ? (
          <p className="flex items-center gap-2 rounded-md border border-emerald-100 bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:border-emerald-400/30 dark:bg-emerald-400/10 dark:text-emerald-200">
        ...
        {mutation.isError ? (
          <div className="rounded-md border border-red-100 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-400/30 dark:bg-red-400/10 dark:text-red-200">
            <div className="flex items-start gap-2">
              <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />
              <div>
                {(mutation.error as AxiosError<{ detail?: string }>)?.response?.data?.detail ?? 'Invalid email or password.'}
                {((mutation.error as AxiosError<{ detail?: string }>)?.response?.data?.detail?.toLowerCase()?.includes('suspended') ||
                  (mutation.error as AxiosError<{ detail?: string }>)?.response?.data?.detail?.toLowerCase()?.includes('pending') ||
                  (mutation.error as AxiosError<{ detail?: string }>)?.response?.data?.detail?.toLowerCase()?.includes('rejected')) && (
                    <Link to="/contact" className="ml-1.5 font-semibold text-red-800 underline transition hover:text-red-950 dark:text-red-300 dark:hover:text-red-100">
                      Support Center
                    </Link>
                  )}
              </div>
