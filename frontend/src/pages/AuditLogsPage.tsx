...
export function AuditLogsPage() {
  ...
  useEffect(() => {
    if (role && role !== 'super_admin' && !user?.feature_audit_logs) {
      setShowPremiumModal(true);
    }
  }, [role, user?.feature_audit_logs]);
  ...
  if (role && role !== 'super_admin' && !user?.feature_audit_logs) {
    return (
      <Page>
        {showPremiumModal ? (
          <PremiumModal
            title="Advanced audit logs require Pro"
            description="Upgrade to unlock full operational audit trails, advanced filtering, and premium dashboards."
            onClose={() => {
        ...
        {logsQuery.isLoading ? (
          ...
          <>
            ...
            <TableFooter
              ...
              itemSummary={`Showing ${visibleLogs.length.toLocaleString()} rows`}
