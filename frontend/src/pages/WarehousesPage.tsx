...
function transferInsight(transfer: StockTransfer) {
  if (transfer.status === 'pending') {
    return 'Waiting for admin approval';
  }
  if (transfer.status === 'approved') {
    return 'Approved and ready to complete';
  }
  ...
  return 'Transfer activity recorded';
}
...
export function WarehousesPage() {
  ...
  const canActOnTransfers = canManage || canApprove;
  ...
  return (
    <Page>
      ...
      <div className={canManage ? 'mt-4 grid gap-4 xl:grid-cols-[0.8fr_1.2fr]' : 'mt-4'}>
        {canManage ? (
          <SectionCard>
                  ...
                  <div key={transfer.id} className="px-4 py-3">
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-sm">
                      ...
                      {canActOnTransfers ? (
                        ...
                        <span className="text-xs font-medium text-slate-500">{transferInsight(transfer)}</span>
                      )}
                    </div>
