      ...
      <div className="mt-8">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {plans.map((plan) => (
            <section
              key={plan.plan_code}
              className={`rounded-lg border bg-white p-5 shadow-sm flex flex-col justify-between relative overflow-hidden ${
                plan.plan_code === 'pro' ? 'border-indigo-200 bg-indigo-50/5' : 'border-slate-200'
              }`}
            >
              {plan.plan_code === 'pro' && (
              ...
              <div>
                ...
                <div className="my-4 border-t border-slate-100 pt-4">
                  <p className="text-3xl font-extrabold text-slate-950">
                    ${plan.price} <span className="text-sm font-normal text-slate-500">/ month</span>
                  </p>
                </div>
                <ul className="space-y-2.5 text-sm text-slate-600">
                  ...
                  <li className={`flex items-center gap-2 ${plan.feature_analytics ? '' : 'text-slate-400'}`}>
                    {plan.feature_analytics ? '✓' : '✗'} Analytics & charts {plan.feature_analytics ? 'enabled' : 'disabled'}
                  </li>
                  <li className={`flex items-center gap-2 ${plan.feature_barcode ? '' : 'text-slate-400'}`}>
                    {plan.feature_barcode ? '✓' : '✗'} Barcode scanner module {plan.feature_barcode ? 'enabled' : 'disabled'}
                  </li>
                  <li className={`flex items-center gap-2 ${plan.feature_audit_logs ? '' : 'text-slate-400'}`}>
                    {plan.feature_audit_logs ? '✓' : '✗'} Audit logs {plan.feature_audit_logs ? 'enabled' : 'disabled'}
                  </li>
                </ul>
              </div>
            </section>
