...
export function UpgradePage() {
  ...
  const plans: PlanCard[] = useMemo(() => {
    const basePlans: PlanCard[] = [
      {
        ...
        priceLabel: '$0 / month',
        bullets: ['Max 5 active users', '2 warehouses maximum', 'Up to 100 catalog SKUs', 'Barcode enabled', 'Analytics enabled', 'Audit logs enabled'],
        cta: 'Current Plan',
      },
    ];
    ...
    const dynamicCards: PlanCard[] = apiPlans.map((p) => {
      ...
      if (p.feature_analytics || p.plan_code === 'free') bullets.push('Analytics enabled');
      if (p.plan_code === 'enterprise') bullets.push('AI forecasting');
      if (p.plan_code === 'pro') bullets.push('AI insights enabled');

      if (p.feature_barcode || p.plan_code === 'free') bullets.push('Barcode enabled');
      if (p.feature_audit_logs || p.plan_code === 'free') bullets.push('Audit logs enabled');
