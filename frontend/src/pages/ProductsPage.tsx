...
import { LABEL_SIZE_OPTIONS, PrintableLabels, getLabelSizeOption, type LabelSize } from '../components/barcode/PrintableLabels';
...
export function ProductsPage() {
  ...
  const listRoute = `${location.pathname}${location.search}`;
  ...
  const printableProducts = selectedProducts.length > 0 ? selectedProducts : products;
  const allVisibleSelected = products.length > 0 && products.every((product) => selectedProductIds.has(product.id));
  const labelSizeOption = getLabelSizeOption(labelSize);
    ...
    <Page>
      <PageHeader
        ...
        actions={
          <>
            ...
            >
              <Barcode className="mr-2 h-4 w-4" /> Print Labels
            </Button>
          </>
        }
      />
      ...
      <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
                ...
                <tr>
                  <DataTableHead>
                    <input
                      ...
                      aria-label="Select all visible products for label printing"
                      className="h-4 w-4 rounded border-slate-300 text-slate-950"
                    />
                  </DataTableHead>
                  ...
                  <DataTableRow key={product.id} className={product.quantity <= 10 ? 'bg-amber-50/35' : undefined}>
                    <DataTableCell>
                      <input
                        ...
                        aria-label={`Select ${product.product_name} for label printing`}
                        className="h-4 w-4 rounded border-slate-300 text-slate-950"
                      />
                    </DataTableCell>
        ...
        <TableFooter
          ...
          itemSummary={`${products.length} of ${total} items shown`}
          onPrevious={prevPage}
          onNext={nextPage}
      ...
      {showPremiumModal ? (
        <PremiumModal
          title="Export reports require Pro"
          description="Upgrade to unlock export-ready compliance reports from Products and Transactions."
      ...
      {showLabelPreview ? (
        <Modal
          ...
          description="Review the printable barcode label sheet before sending it to your printer."
          onClose={() => setShowLabelPreview(false)}
          className="max-w-6xl"
        >
          <div className="no-print border-b border-slate-200 bg-slate-50/80 px-5 py-4">
            <div className="grid gap-3 md:grid-cols-4">
              <div className="rounded-lg border border-slate-200 bg-white p-3">
                <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Total labels</p>
                <p className="mt-1 text-2xl font-semibold text-slate-950">{printableProducts.length.toLocaleString()}</p>
              </div>
              <div className="rounded-lg border border-slate-200 bg-white p-3">
                <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Selected products</p>
                <p className="mt-1 text-2xl font-semibold text-slate-950">{selectedProducts.length.toLocaleString()}</p>
              </div>
              <div className="rounded-lg border border-slate-200 bg-white p-3">
                <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Print source</p>
                <p className="mt-1 text-sm font-semibold text-slate-950">
                  {selectedProducts.length > 0 ? 'Selected products' : 'Current filtered results'}
                </p>
              </div>
              <label className="rounded-lg border border-slate-200 bg-white p-3">
                <span className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Label size</span>
                <Select
                  value={labelSize}
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
              <p className="text-sm text-slate-500">
                Label size: {labelSizeOption.label} ({labelSizeOption.dimensions})
              </p>
              <div className="flex gap-2">
                <Button type="button" className="bg-slate-700 hover:bg-slate-600" onClick={() => setShowLabelPreview(false)}>
                  Cancel
                </Button>
                <Button type="button" onClick={printLabels} disabled={printableProducts.length === 0}>
          ...
          <div className="max-h-[68vh] overflow-auto bg-slate-100 p-4 sm:p-6">
            {printableProducts.length === 0 ? (
              <EmptyState title="No labels to print" description="Select products or apply filters that return products." />
            ) : (
              <PrintableLabels products={printableProducts} labelSize={labelSize} preview />
            )}
          </div>
        </Modal>
      ) : null}
