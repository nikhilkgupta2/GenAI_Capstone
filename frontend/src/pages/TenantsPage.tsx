...
export function TenantsPage() {
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setNotice(`Tenant status updated to ${data.status.toUpperCase()}.`);
    },
  });
  ...
  const planMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['super-admin', 'tenants'] });
      setNotice('Tenant plan and subscription limits updated.');
  ...
  const featuresMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['super-admin', 'tenants'] });
      setNotice('Tenant feature flags updated.');
                  ...
                  return (
                    <DataTableRow key={t.id}>
                      <DataTableCell className="max-w-[250px]">
                        <div className="font-semibold text-slate-900 break-words">{t.company_name}</div>
                      ...
                      <DataTableCell>
                        <div className="flex flex-wrap gap-2">
                          <Link
                            to={`/tenants/${t.id}`}
                            className="inline-flex items-center rounded-md border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition"
                          ...
                          >
                            <Edit2 className="mr-1 h-3.5 w-3.5" /> Plan
                          </button>

                          <button
                            type="button"
                            className="inline-flex items-center rounded-md border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition"
                            onClick={() => openFeaturesModal(t)}
                          >
                            <Layers className="mr-1 h-3.5 w-3.5" /> Features
                          </button>
