...
class Notification(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    def priority(self) -> str:
        if self.type in {"out_of_stock", "stockout_risk", "capacity_exceeded", "system_error"}:
            return "critical"
        if self.type in {"low_stock", "shipment_delayed", "supplier_performance_drop", "dead_stock_detected"}:
            return "high"
        if self.group in {"procurement", "suppliers", "ai", "audit"}:
            return "medium"
        return "normal"
