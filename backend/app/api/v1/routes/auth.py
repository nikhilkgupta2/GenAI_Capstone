...
@router.post("/support", response_model=ApiResponse, status_code=status.HTTP_201_CREATED)
def submit_support_request(payload: SupportRequestCreate, db: Session = Depends(get_db)) -> ApiResponse:
    support_request = SupportRequest(**payload.model_dump())
    db.add(support_request)
    db.commit()
    db.refresh(support_request)
    return ApiResponse(message="Support request submitted successfully.", data={"id": str(support_request.id)})
