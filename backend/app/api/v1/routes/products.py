...
def create_product(
    ...
    if tenant_id:
        ...
        if tenant:
            service = ProductService(db)
            current_count = service.count(tenant_id=tenant_id)
            if current_count >= tenant.max_products:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail=f"Product limit reached ({tenant.max_products}). Please upgrade your plan."
                )
    service = ProductService(db)
