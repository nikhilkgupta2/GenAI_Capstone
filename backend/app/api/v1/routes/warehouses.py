...
def create_warehouse(payload: WarehouseCreate, db: DbSession, current_user: WarehouseManager) -> ApiResponse:
    if current_user.tenant_id:
        ...
        if tenant:
            current_count = len(WarehouseService(db).list_warehouses(current_user.tenant_id))
            if current_count >= tenant.max_warehouses:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail=f"Warehouse limit reached ({tenant.max_warehouses}). Please upgrade your plan."
                )
