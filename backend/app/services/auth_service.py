...
class AuthService:
    ...
    def register(self, payload: RegisterRequest) -> AuthResponse | RegisterPendingVerificationResponse:
        ...
        if existing and existing.is_email_verified:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="email already exists : try another email")

        # Also check if a tenant with this email already exists separately
        existing_tenant = self.tenants.get_by_contact_email(normalized_email)
        if existing_tenant and not existing:
             raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="email already exists : try another email")
