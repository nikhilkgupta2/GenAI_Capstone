...
class RAGService:
    ...
    def initial_ingestion(self):
        ...
        for doc in docs_to_index:
            path1 = os.path.abspath(os.path.join(os.getcwd(), "..", doc))
            path2 = os.path.abspath(os.path.join(os.getcwd(), "..", "..", doc))
            path = path1 if os.path.exists(path1) else path2
            if os.path.exists(path):
                print(f"Indexing {doc} from {path}...")
                self.ingestion.ingest_text_file(path)
            else:
                print(f"File not found (tried {path1} and {path2})")
